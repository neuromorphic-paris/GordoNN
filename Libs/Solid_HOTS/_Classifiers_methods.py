...
# =============================================================================      
def hist_classification_test(self, labels, labels_test, number_of_labels):
    
    """
    Method to train and test a simple histograms classificator
    ...
    n_events_test=np.zeros([len(labels_trim_test)])
    ...
    for i,cl in enumerate(labels_trim): 
        for event in range(len(last_layer_activity[i][1])):
            ...
            n_events[i] += 1
        cl_hist_norm[i]=cl_hist[i]/n_events[i]
...
def hist_mlp_classification_test(self, labels, number_of_labels, batch_size, threshold):
    ...
    for i,cl in enumerate(labels_trim): 
        mlp_labels_test[i,cl] = 1
        for event in range(len(last_layer_activity[i][1])):
            cl_hist_test[i,last_layer_activity[i][1][event]] += 1
...
def lstm_classification_train(self, dataset_train, labels, dataset_test, labels_test, number_of_labels, bin_width, 
    ...
    #         sonogram_labels_bins_train.append(labels[i])
    ...
    #         sonogram_labels_bins_test.append(labels_test[i])
    ...
    # # fit model
    # self.sonogram_lstm.fit(np.array(sonogram_activity_binned_train), np.array(labels),
    #   epochs=epochs,
    #   batch_size=batch_size, validation_data=(np.array(sonogram_activity_binned_test), np.array(labels_test)),
...
def lstm_classification_test(self, dataset_train, labels, dataset_test, labels_test, number_of_labels, bin_width, 
    ...
    # prediction_rate_train_sonogram=sum(labels==np.around(np.reshape(predicted_labels_sonogram_ev_train,len(predicted_labels_sonogram_ev_train))))/len(labels)
    # prediction_rate_test_sonogram=sum(labels_test==np.around(np.reshape(predicted_labels_sonogram_ev_test,len(predicted_labels_sonogram_ev_test))))/len(labels_test)

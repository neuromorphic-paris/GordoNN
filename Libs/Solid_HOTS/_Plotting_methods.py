...
def plt_surfaces_vs_reconstructions(self, file, layer=0, test=False):
    ...
    encoded_lc_surfaces=self.aenc_T[layer][1].predict(np.asarray(original_lc_surfaces))
    reconstr_lc_surfaces=self.aenc_T[layer][2].predict(np.asarray(encoded_lc_surfaces))
    ...
    # Reconstructing the local time surfaces
    encoded_surfaces=self.aenc_2D[layer][1].predict(original_surfaces)
    reconstr_surfaces=self.aenc_2D[layer][2].predict(encoded_surfaces)
...
def plt_reverse_activation(self, file, layer, sublayer, labels, labels_test,
    ...
    n = self.features_number[layer][sublayer]
    
    if not sublayer:
        for feature in range(n):
            # Extract events by feature
            rel_indx = [i for i, x in enumerate(sublayer_data[file][2]) if x == feature]
            timestamps = [orig_timestamps[i] for i in rel_indx]
            channels = [orig_channels[i] for i in rel_indx]
            values = [sublayer_data[file][3][i] for i in rel_indx]
            plt.figure()
            plt.suptitle('Layer: '+str(layer)+' 0T-response File: '+ str(file) +' Class: '+ file_class, fontsize=16)
            image = plt.scatter(timestamps,channels, c=values, s=2, vmin = 0, vmax = 1)
            plt.xlim(xlims)
            plt.ylim(ylims)
            plt.colorbar(image)
        ...
        for feature in range(n):
            # Values in 2D responses are packed in a n-long array with a single
            # timestamp
            values = [sublayer_data[file][1][i][feature] for i in range(len(abs_indx))]
            plt.figure()
            plt.suptitle('Layer: '+str(layer)+' 2D-response File: '+ str(file) +' Class: '+ file_class, fontsize=16)
            image = plt.scatter(orig_timestamps, orig_channels, c=values, s=2, vmin = 0, vmax = 1)
            plt.xlim(xlims)
            plt.ylim(ylims)
            plt.colorbar(image)   

...
# =============================================================================
def events_from_activations(features, context_indices, events):
    """
    Function used to compute events out of simple activations (raw values)
    for a single recording
    Arguments :
...
def recording_surface_generator(recording, dataset, polarities, features_number, 
    ...
    all_surfaces_recording = np.zeros([len(dataset[recording][0]),xdim*ydim], dtype="float16")
    context_indices = np.zeros([len(dataset[recording][0])], dtype="int")
    timestamps = np.zeros([ydim, xdim], dtype=int)
    timesurf = np.zeros([ydim, xdim], dtype="float16")

    count = 0 
    old_timestamp = dataset[recording][0][0]       
    for event_ind in range(0, len(dataset[recording][0])):   
        ...
        feature = dataset[recording][2][event_ind]
        timestamps[polarity, feature] = dataset[recording][0][event_ind]
        timesurf = np.exp(-(new_timestamp-timestamps)/taus_2D)*(timestamps!=0)   

...
class Solid_HOTS_Net:
    ...
    def learn(self, dataset, dataset_test, labels, labels_test, num_classes, rerun_layer=0):
        ...
        for layer in layers_index: 
            ...
            all_surfaces=[[] for i in range(num_classes)] 
            surface_indices=[]
            all_surfaces_test=[[] for i in range(num_classes)]
            ...
            # unpacking the results                   
            for recording in range(len(net_0D_response)):                    
                all_surfaces[labels[recording]].append(results[recording][1])
                surface_indices.append(results[recording][0])
           
            all_surfaces = [np.concatenate(all_surfaces[i], axis=0) for i in range(num_classes)]
            ...
            # unpacking the results                   
            for recording in range(len(net_0D_response_test)):                    
                all_surfaces_test[labels_test[recording]].append(results[recording][1])
                surface_indices_test.append(results[recording][0])
                
            all_surfaces_test = [np.concatenate(all_surfaces_test[i], axis=0) for i in range(num_classes)] 
            ...
            # The final results of the local surfaces test dataset computation
            all_surfaces = [np.array(all_surfaces[i], dtype='float16') for i in range(num_classes)]
            all_surfaces_test = [np.array(all_surfaces_test[i], dtype='float16') for i in range(num_classes)]
            gc.collect()
            ...
            for i in range(num_classes):
                kmeans_methods.append(KMeans(n_clusters=self.features_number[layer][1], verbose=1).fit(all_surfaces[i]))
                kmeans_centers = [*kmeans_centers, *kmeans_methods[i].cluster_centers_]
            kmeans_layer = kmeans_methods[0]
            kmeans_layer.cluster_centers_ = np.array(kmeans_centers)
            kmeans_layer.set_params(n_clusters=self.features_number[layer][1]*num_classes)
            self.aenc_2D.append(kmeans_layer)
            ...
            current_pos = [0 for i in range(num_classes)]
            current_pos_test = [0 for i in range(num_classes)]
            ...
            for recording in range(len(surface_indices)):
                ...
                recording_results = self.aenc_2D[layer].predict(np.array(all_surfaces[label][current_pos[label]:current_pos[label]+len(surface_indices[recording])]))
                current_pos[label] += len(surface_indices[recording])
            ...
            for recording in range(len(surface_indices_test)):
                ...
                recording_results = self.aenc_2D[layer].predict(np.array(all_surfaces_test[label][current_pos_test[label]:current_pos_test[label]+len(surface_indices_test[recording])]))
                current_pos_test[label] += len(surface_indices_test[recording])
            ...
            self.tmp=[all_local_surfaces, all_local_surfaces_test, all_local_surfaces, all_surfaces_test]

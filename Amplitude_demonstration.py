...
#Original model of magnitude offset
plt.figure()
orig_mag_model = np.linspace(9,2,32)
...
new_mag_model_log = np.log(actual_freq)*m + B
new_mag_model = np.exp(new_mag_model_log)
plt.plot(np.log(actual_freq), np.log(new_mag_model), label="new wrong approximation")
plt.legend()



plt.plot(np.log(actual_freq), np.log(mean_rate), label="Actual Magnitude attenuation")
plt.legend()
plt.ylabel("log magnitude offs")
plt.xlabel("log frequency")
plt.title("Magnitude offset (it should be a straight line)")
...
plt.plot(np.log(actual_freq), orig_mag_model, label="original approximation")


plt.plot(np.log(actual_freq), new_mag_model, label="new wrong approximation")


plt.plot(np.log(actual_freq), mean_rate, label="Actual Magnitude attenuation")
plt.legend()
plt.ylabel("magnitude offs")
plt.xlabel("log frequency")
plt.title("Magnitude offset (it should be a straight line)")
...
#cochlea channels    
freq = freq[18:]#cut high freq channels to avoid problems with the nyquist frequency
mean_rate = mean_rate[18:]/np.mean(mean_rate[18:])#cut high freq channels to avoid problems with the nyquist frequency

y = amp*np.sin(2*np.pi*np.outer(freq,np.arange(0,simulation_time)))
y = np.transpose(mean_rate*np.transpose(y))
